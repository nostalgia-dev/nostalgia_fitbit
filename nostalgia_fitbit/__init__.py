from nostalgia_fitbit._version import __version__, version
...
class FitbitAuth(object):
    ...
    def ensure_access_token(self):
        ...
        if just.exists(self.ACCESS_TOKEN_FILE):
            access_token = just.read(self.ACCESS_TOKEN_FILE, unknown_type="json")
...
class Fitbit(object):
    ...
    def get_sleep(self, date):
        r = self.session.get(
            'https://api.fitbit.com/1.2/user/{}/sleep/date/{}.json'.format(self.user_id, str(date))
        )
        r.raise_for_status()
    ...
    def get_sleep_range(self, date_start, date_end):
        r = self.session.get(
            'https://api.fitbit.com/1.2/user/{}/sleep/date/{}/{}.json'.format(
                self.user_id, str(date_start), str(date_end)
            )
        )
        r.raise_for_status()
        return json.loads(r.text)['sleep']
    ...
    def get_heartrate_intraday(self, date):
        r = self.session.get(
            'https://api.fitbit.com/1/user/-/activities/heart/date/{}/{}/1sec.json'.format(
                str(date), "1d"
            )
        )
        r.raise_for_status()
